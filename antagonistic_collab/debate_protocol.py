...
# ---------------------------------------------------------------------------
# Parametric design space — extends the fixed registry with structures
# generated by varying continuous parameters of existing generators.
# Activated by the richer_design_space config (default: on).
#
# Rationale: a continuous design space lets EIG find the diagnostic sweet
# spot that fixed structures may miss (Myung & Pitt 2009; Cavagnaro et al.
# 2010). For example, intermediate cluster separation reveals model
# differences that extreme values (too easy / too hard) mask.
# ---------------------------------------------------------------------------


def _build_parametric_structures() -> dict[str, dict]:
    """Generate structures by varying parameters of existing generators.

    Deterministic seeds ensure reproducibility. Names encode parameters
    so they can be resolved by _synthetic_runner.
    """
    structs: dict[str, dict] = {}

    # Linear separable: vary separation and dimensionality.
    # Existing registry has (2D, sep=2.0) and (4D, sep=2.0).
    # We fill in: low/high separation × varied dimensionality.
    _ls_params = [
        (2, 1.0, 300),  # (n_dims, separation, seed)
        (2, 3.0, 301),
        (3, 1.5, 302),
        (3, 2.5, 303),
        (4, 1.0, 304),
    ...
    for n_dims, sep, seed in _ls_params:
        name = f"linear_separable_{n_dims}d_sep{sep:.1f}"
        structs[name] = linear_separable(n_dims=n_dims, separation=sep, seed=seed)

    # Rule-plus-exception: vary dimensionality and exception count.
    # Existing registry has (4D, 1exc) and (4D, 2exc).
    _rpe_params = [
        (3, 1, 400),  # (n_dims, n_exceptions, seed)
        (3, 2, 401),
        (5, 1, 402),
        (5, 2, 403),
        (6, 1, 404),
    ...
    for n_dims, n_exc, seed in _rpe_params:
        name = f"rule_plus_exception_{n_dims}d_{n_exc}exc"
        structs[name] = rule_plus_exception(
            n_dims=n_dims, n_exceptions=n_exc, seed=seed
        )
...
def _build_parametric_conditions() -> dict[str, dict[str, dict]]:
    ...
    These fill gaps in the condition space — moderate attention sits between
    low and high; mild noise sits between baseline and high_noise.
    """
    return {
        "moderate_attention": {
            "GCM": {"c": 3.5},  # midpoint of low (1.5) and high (6.0)
            "SUSTAIN": {"r": 7.5},  # midpoint of low (3.0) and high (12.0)
            "RULEX": {"p_single": 0.5, "p_conj": 0.28},
        },
        "mild_noise": {
            "GCM": {"c": 3.0},  # between baseline (4.0) and high_noise (2.0)
            "SUSTAIN": {"eta": 0.07, "r": 7.0},
            "RULEX": {"error_tolerance": 0.15},
        },
    }


PARAMETRIC_CONDITIONS: dict[str, dict[str, dict]] = _build_parametric_conditions()
...
class DebateProtocol:
    ...
    def compute_model_predictions(
        ...
        """
        # Resolve structure: check registry, parametric, then temporary, then
        ...
        if condition in all_conditions:
            model_key = agent_config.model_class.name.split()[0]
            condition_params.update(all_conditions[condition].get(model_key, {}))
        fallback_params = {
    ...
    def _synthetic_runner(
        ...
        if true_model == "GCM":
            ...
            overrides = all_conditions[condition].get("GCM", {})
        elif true_model == "SUSTAIN":
            model = SUSTAIN()
            params = {"r": 9.01, "beta": 1.252, "d": 16.924, "eta": 0.092}
            overrides = all_conditions[condition].get("SUSTAIN", {})
            ...
            overrides = all_conditions[condition].get("RULEX", {})
        else:
