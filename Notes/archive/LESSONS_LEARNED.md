...
**Implication:** Even with explicit format examples and menus, LLMs prefer semantic expressiveness over structured format compliance. This is consistent with the broader literature: Tam et al. (EMNLP 2024) show that format restrictions actively degrade reasoning performance — the constraint interferes with generation, not just parsing. The IFEval benchmark (Zhou et al. 2023) finds no model exceeds 80% on verifiable format constraints; a 256-model study found 43.7% overall pass rate. Schall & de Melo (RANLP 2025) show instruction-tuned models drop 17% accuracy under constrained decoding. The mixture distribution design was critical: it works with low parsing rates because even 1 parseable crux per run can direct an experiment. Robust pipelines should assume <50% format compliance and design mechanisms that degrade gracefully. Constrained decoding or fuzzy post-processing are the only reliable alternatives.
...
**Implication:** The theoretical 30% rate assumes cruxes reference experiments outside the EIG frontier. In practice, agents identify cruxes that overlap with already-high-EIG candidates. This convergence between semantic and computational experiment selection was predicted by Corcoran, Hohwy & Friston (2023, *Neuron*), who argue that adversarial collaboration and Bayesian optimal design should be unified because both target the same discriminating experiments. Ouyang et al. (2018) found a similar pattern: Medin & Schaffer's (1978) intuitively designed 5-4 structure happened to place competing models near maximal EIG divergence. The unique value of crux-directed selection appears when cruxes point to experiments that EIG undervalues, which happened once (rule_plus_exception_1exc/high_noise was not the top EIG candidate).
...
### On cross-domain replication (Decision M15)

**61. RMSE gates must validate against accumulated data, not current-cycle data.** The first decision M15 run validated revisions against only 2-3 current-cycle gambles. Competitor agents gamed this by proposing revisions that improved local RMSE while hurting global fit. EU-GT debate was perversely worse than no-debate because PH accepted 8/8 locally-valid revisions. After switching to accumulated observations (all gambles seen so far), acceptance dropped from 79% to 49% and the perverse outcome disappeared. This is the RMSE-gate version of "calibrate against real pipeline quantities, not toy tests." (Phase 25, D50)

**62. LLM parameter recovery depends on the intuitive interpretability of parameters.** Decision M15: PH recovered 81.8% (rule thresholds are intuitive: "be less sensitive"), CPT recovered 51% (lambda_=loss aversion is intuitive but alpha/beta=value function curvature is not), EU recovered 0% (too abstract). In categorization M15: RULEX recovered 60% (rule probabilities are intuitive), GCM recovered 86% (sensitivity c is intuitive), SUSTAIN recovered 0% (cluster dynamics are opaque). Pattern: parameters that map to linguistically describable behaviors get recovered; parameters that require mathematical understanding don't. (Phase 25, D51)
...
**64. Cross-domain replication is partial, not complete.** Categorization M15: 0/3→2/3 debate correct. Decision M15: 0/3→1/3. The weaker replication has two identified causes: (a) decision models make sharper predictions, so posteriors lock in faster with less room for debate correction; (b) CPT has 5 params vs GCM's 3, with some (alpha/beta) outside LLM diagnostic capability. The core mechanism (debate recovers rule-based models via parameter diagnosis) replicates; the magnitude doesn't. (Phase 25, D51)
